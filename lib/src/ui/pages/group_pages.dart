...
class GroupDetailPageState extends State<GroupDetailPage> {
  ...
  bool _memberStatusIsError = false;

  @override
  void initState() {
    super.initState();
    _groupName = widget.group.name;
    _notice = widget.group.subtitle;
    _inviteConfirm = widget.group.inviteConfirm;
    _saved = widget.group.saved;
    _muted = widget.group.muted;
    // 不再用 seedContacts 预填 5 个 mock 成员当占位 (加载时会闪假数据)。
    // 留空 → _loadGroupMembers 期间走 _membersLoading 占位, 拿到真实成员再填。
  ...
  Widget build(BuildContext context) {
    ...
    return DetailScaffold(
      ...
      children: [
            ...
            Row(
              children: [
                MoyuResolvedAvatar.raw(
                  label: group.avatarLabel,
                  size: 64,
                  colors: [group.color, MoyuColors.of(context).primarySoft],
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 5),
                      Text(
                        t.groupMemberCount(group.memberCount),
                        style: TextStyle(
                          color: MoyuColors.of(context).textTertiary,
                          fontSize: 13,
                        ),
        ...
        SectionTitle(t.groupMembersSection),
        MoyuSection(
          ...
          children: [
            if (_membersLoading)
              ...
              Wrap(
                ...
                runSpacing: 12,
                children: [
                  for (final contact in _membersByRole().take(8))
                    SizedBox(
                      width: 58,
                      child: Column(
                        children: [
            ...
            StaticActionRow(
              ...
              title: t.groupInviteMembers,
              subtitle: t.groupInviteMembersSubtitle,
              onTap: () => unawaited(_inviteMembers()),
            ),
            const MoyuDivider(),
            StaticActionRow(
              icon: FIcons.userMinus,
              iconColor: MoyuColors.of(context).red,
              title: t.groupRemoveMembers,
              subtitle: _visibleMembers.isEmpty
                  ? t.groupRemoveMembersEmptySubtitle
                  : t.groupRemoveMembersSubtitle,
              onTap: _visibleMembers.isEmpty
                  ? null
                  : () => unawaited(_removeMembers()),
            ),
            if (_memberStatus.isNotEmpty)
              Padding(
                padding: const EdgeInsets.fromLTRB(2, 10, 2, 0),
                child: Text(
                  _memberStatus,
                  style: TextStyle(
                    color: _memberStatusIsError
        ...
        MoyuSection(
          ...
          children: [
            StaticActionRow(
              ...
              subtitle: t.groupQrCodeSubtitle,
              onTap: () => pushPage(
                context,
                GroupQrCodePage(
                  group: group,
            ...
            StaticActionRow(
              icon: FIcons.pencil,
              iconColor: MoyuColors.of(context).primary,
              title: t.groupNameTitle,
              subtitle: _groupName,
              value: t.actionEdit,
              onTap: () => _openGroupTextEditor(
                title: t.groupNameTitle,
                fieldKey: 'name',
            ...
            StaticActionRow(
              ...
              subtitle: _notice.isEmpty ? t.groupNoticeUnset : _notice,
              onTap: () => pushPage(
                context,
                GroupNoticeListPage(
                  ...
                  canEdit: false,
                  socialGateway: widget.socialGateway,
                  onSaved: (value) => setState(() => _notice = value),
                ),
              ),
            ),
            const MoyuDivider(),
            StaticActionRow(
              ...
              subtitle: t.groupManageSubtitle,
              onTap: () => pushPage(
                context,
                GroupManagePage(
            ...
            SwitchRow(
              ...
              onChanged: (value) =>
                  unawaited(_updateGroupInfoSwitch('invite', value)),
            ),
            const MoyuDivider(),
            StaticActionRow(
              icon: FIcons.ban,
              ...
              subtitle: t.groupBlacklistSubtitle,
              onTap: () => pushPage(
                context,
                GroupBlacklistPage(
                  group: group,
            ...
            const MoyuDivider(),
            SwitchRow(
              title: t.groupSaveToContacts,
              value: _saved,
              onChanged: (value) =>
                  unawaited(_updateGroupSetting('save', value)),
            ),
            const MoyuDivider(),
            SwitchRow(
              title: t.groupMuteMessages,
              value: _muted,
              onChanged: (value) =>
                  unawaited(_updateGroupSetting('mute', value)),
            ),
          ],
        ),
        ...
        Padding(
          ...
          child: _leftGroup
              ...
              : MoyuDangerButton(
                  label: t.groupExitAction,
                  onPressed: () => unawaited(_exitGroup()),
                ),
        ),
      ],
    );
  ...
  List<UiContact> get _contactChoices => contactsAllowedForGroupInvite(
    widget.contacts, // 不再 fallback seedContacts mock
    widget.serverAppConfig,
  );

  List<UiContact> get _visibleMembers => _members;

  /// Sort members so creator > managers > regular members; preserve
  /// original order within each role bucket (server returns by join
  /// time). Server role enum (modules/group/const.go):
  ///   0 = Common, 1 = Creator, 2 = Manager
  /// Plain `b.role.compareTo(a.role)` would put manager(2) before
  /// creator(1) — wrong. Map to a priority axis first.
  List<UiContact> _membersByRole() {
    int prio(int role) {
      if (role == 1) return 2; // creator on top
      if (role == 2) return 1; // managers next
      return 0;
    }

    final sorted = [..._visibleMembers];
  ...
  Future<void> _loadGroupMembers() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _memberStatus = t.groupMembersSyncFailed('$error');
          _memberStatusIsError = true;
        });
      }
    } finally {
      if (mounted) {
  ...
  Future<void> _inviteMembers() async {
    final t = AppLocalizations.of(context);
    final memberIds = {
      for (final member in _visibleMembers) contactChannelId(member),
    };
    final candidates = [
      for (final contact in _contactChoices)
        if (!memberIds.contains(contactChannelId(contact))) contact,
    ];
    final selected = await Navigator.of(context).push<List<UiContact>>(
      MaterialPageRoute(
        builder: (_) => MoyuContactPickerPage(
          title: t.groupInvitePickerTitle,
          contacts: candidates,
    ...
    try {
      if (_inviteConfirm) {
        await widget.socialGateway?.inviteGroupMembers(
          groupNo: _groupNo,
          memberUids: memberUids,
        );
      } else {
        await widget.socialGateway?.addGroupMembers(
          groupNo: _groupNo,
          memberUids: memberUids,
        );
      ...
      setState(() {
        if (_inviteConfirm) {
          _memberStatus = t.groupInviteSentCount(selected.length);
        } else {
          _memberStatus = t.groupInvitedCount(selected.length);
        }
        _memberStatusIsError = false;
      });
    } catch (error) {
      if (mounted) {
        setState(() {
          _memberStatus = t.groupInviteMembersFailed('$error');
          _memberStatusIsError = true;
        });
      }
    }
  ...
  Future<void> _removeMembers() async {
    ...
    final selected = await Navigator.of(context).push<List<UiContact>>(
      MaterialPageRoute(
        builder: (_) => MoyuContactPickerPage(
          title: t.groupRemovePickerTitle,
          contacts: _visibleMembers,
          selectedContacts: const [],
        ),
      ),
    );
    ...
    // DESIGN.md §6：移除成员不可撤销，picker 选完后必须 confirm。
    final namesPreview = selected.length == 1
        ? t.groupQuotedMemberName(selected.first.name)
        : t.groupSelectedMemberCount(selected.length);
    var confirmed = false;
    await MoyuActionSheet.show(
      context,
      title: t.groupRemoveMembersConfirm(namesPreview),
      items: [
        MoyuActionSheetItem(
          title: t.groupRemoveAction,
          destructive: true,
    ...
    final memberUids = _contactIds(selected);
    try {
      await widget.socialGateway?.removeGroupMembers(
        groupNo: _groupNo,
        memberUids: memberUids,
      );
      if (!mounted) {
      ...
      final removedIds = memberUids.toSet();
      setState(() {
        _members.removeWhere(
          (member) => removedIds.contains(contactChannelId(member)),
        );
        _memberStatus = t.groupRemovedCount(selected.length);
        _memberStatusIsError = false;
      });
    } catch (error) {
      if (mounted) {
        setState(() {
          _memberStatus = t.groupRemoveMembersFailed('$error');
          _memberStatusIsError = true;
        });
      }
    }
  ...
  List<String> _contactIds(List<UiContact> contacts) {
    return [
      for (final contact in contacts)
        if (contactChannelId(contact).isNotEmpty) contactChannelId(contact),
    ];
  }

  void _openGroupTextEditor({
  ...
  Future<void> _updateGroupInfoSwitch(String key, bool value) async {
    final t = AppLocalizations.of(context);
    _setLocalSwitch(key, value);
    try {
      if (widget.socialGateway != null && widget.group.groupNo.isNotEmpty) {
        await widget.socialGateway!.updateGroupInfo(
          groupNo: widget.group.groupNo,
          key: key,
          value: value ? '1' : '0',
        );
        // 修"开关重进恢复"BUG — 必须刷 SDK channel info, 否则上层
        // group_list / chat_settings 重进 widget.group 都是 stale.
        await ChatImGateway.current?.refreshChannel(
          channelId: widget.group.groupNo,
          channelType: 2,
        );
      }
      if (mounted) {
        setState(() => _status = t.groupSettingsUpdated);
      }
    } catch (error) {
      _setLocalSwitch(key, !value);
      if (mounted) {
        setState(() => _status = t.groupSettingsUpdateFailed('$error'));
      }
    }
  }

  Future<void> _updateGroupSetting(String key, bool value) async {
    final t = AppLocalizations.of(context);
    _setLocalSwitch(key, value);
    try {
      if (widget.socialGateway != null && widget.group.groupNo.isNotEmpty) {
        ...
        // 修"开关重进恢复"BUG — 同 _updateGroupInfoSwitch.
        await ChatImGateway.current?.refreshChannel(
          channelId: widget.group.groupNo,
          channelType: 2,
        );
  ...
  Future<void> _exitGroup() async {
    final t = AppLocalizations.of(context);
    // DESIGN.md §6：危险动作必须 confirm。退群一旦执行不可撤销
    // （需要重新邀请才能加回），直接按钮 -> 调用是手滑灾难。
    await MoyuActionSheet.show(
      context,
      title: t.groupExitConfirm,
      items: [
        MoyuActionSheetItem(
          title: t.groupExitAction,
          destructive: true,
          onSelected: () => unawaited(_doExitGroup()),
        ),
      ],
    );
  }

  Future<void> _doExitGroup() async {
    final t = AppLocalizations.of(context);
    final groupNo = widget.group.groupNo.isEmpty
        ? widget.group.name
        : widget.group.groupNo;
    try {
      await widget.socialGateway?.exitGroup(groupNo);
      if (!mounted) return;
      setState(() => _leftGroup = true);
      MoyuToast.show(context, t.groupExitSuccess);
    } catch (error) {
      if (mounted) {
        MoyuToast.show(context, t.groupExitFailed('$error'));
      }
    }
  }
}
...
class GroupManagePageState extends State<GroupManagePage> {
  ...
  Future<void> _syncAfterSwitch() async {
    // fallback 到全局单例 — GroupListPage → GroupDetailPage → GroupManagePage
    // 三层 widget 不便都 thread imGateway, 用 ChatImGateway.current.
