...
class WKDBHelper {
  ...
  Future<bool> init() async {
    ...
    String path = p.join(databasesPath, 'wk_${_databaseScope()}.db');
  ...
  Future<bool> onUpgrade(Database db) async {
    ...
    String scope = _databaseScope();
    int maxVersion = preferences.getInt('wk_max_sql_version_$scope') ?? 0;
    int saveVersion = 0;
    ...
    if (saveVersion > 0) {
      preferences.setInt('wk_max_sql_version_$scope', saveVersion);
    }
    return true;
  }
  ...
  String _databaseScope() {
    final scope = WKIM.shared.options.databaseScope?.trim() ?? '';
    if (scope.isNotEmpty) return scope;
    return WKIM.shared.options.uid ?? '';
  }
}
