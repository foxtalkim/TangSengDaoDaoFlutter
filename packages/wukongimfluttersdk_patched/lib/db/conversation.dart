...
class ConversationDB {
  ...
  Future<WKUIConversationMsg?> insertOrUpdateWithConvMsg(
    ...
    if (lastMsg == null || lastMsg.channelID.isEmpty) {
      ...
      if (!shouldReplaceLiveLastMessage(lastMsg, conversationMsg)) {
        return null;
      }
      // 这里有错误数据，需要清理
  ...
  static bool shouldReplaceLiveLastMessage(
      WKConversationMsg existing, WKConversationMsg incoming) {
    if (existing.channelID.isEmpty) {
      return true;
    }
    if (incoming.lastMsgSeq > 0 && existing.lastMsgSeq > 0) {
      return incoming.lastMsgSeq > existing.lastMsgSeq;
    }
    if (incoming.lastMsgTimestamp != existing.lastMsgTimestamp) {
      return incoming.lastMsgTimestamp > existing.lastMsgTimestamp;
    }
    if (incoming.lastMsgSeq != existing.lastMsgSeq) {
      return incoming.lastMsgSeq > existing.lastMsgSeq;
    }
    if (incoming.lastClientMsgNO.isNotEmpty &&
        existing.lastClientMsgNO.isEmpty) {
      return true;
    }
    return false;
  }
