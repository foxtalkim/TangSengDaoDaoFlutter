...
class MessageDB {
  ...
  final String extraCols =
      "IFNULL(${WKDBConst.tableMessageExtra}.readed,0) as readed,IFNULL(${WKDBConst.tableMessageExtra}.readed_count,0) as readed_count,IFNULL(${WKDBConst.tableMessageExtra}.unread_count,0) as unread_count,IFNULL(${WKDBConst.tableMessageExtra}.reply_count,0) as reply_count,IFNULL(${WKDBConst.tableMessageExtra}.revoke,0) as revoke,IFNULL(${WKDBConst.tableMessageExtra}.revoker,'') as revoker,IFNULL(${WKDBConst.tableMessageExtra}.extra_version,0) as extra_version,IFNULL(${WKDBConst.tableMessageExtra}.is_mutual_deleted,0) as is_mutual_deleted,IFNULL(${WKDBConst.tableMessageExtra}.need_upload,0) as need_upload,IFNULL(${WKDBConst.tableMessageExtra}.content_edit,'') as content_edit,IFNULL(${WKDBConst.tableMessageExtra}.edited_at,0) as edited_at,IFNULL(${WKDBConst.tableMessageExtra}.is_pinned,0) as is_pinned";
        ...
        for (WKMsgExtra extra in msgExtraList) {
          for (int i = 0, size = msgList.length; i < size; i++) {
            ...
            if (extra.contentEdit != '' &&
              ...
              // FIX: 原为 jsonEncode — contentEdit 本就是 JSON 字符串, 再 encode
              // 成带转义的 String 传给期望 Map 的 readInt (data[key]) →
              // "String is not a subtype of int of index" 抛异常, 整个
              // getMessages 失败 → 含"编辑过的引用消息"的会话进入时消息加载不出
              // /空白 (只靠 stream 推送才慢慢灌回来)。对齐 const.dart 正确用法:
              // jsonDecode 成 Map 再用 (外层已判 contentEdit != '', 这里防 null)。
              final editJson = jsonDecode(extra.contentEdit);
              if (editJson != null) {
                final type = WKDBConst.readInt(editJson, 'type');
                msgList[i].messageContent!.reply!.contentEditMsgModel =
                    WKIM.shared.messageManager.getMessageModel(type, editJson);
              }
              break;
            }
          }
  ...
  dynamic getExtraMap(WKMsgExtra extra) {
    ...
    map['reply_count'] = extra.replyCount;
